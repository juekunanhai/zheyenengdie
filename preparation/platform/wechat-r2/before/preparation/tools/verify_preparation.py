...
has_playful_r3=(ROOT/'audio/PLAYFUL_R3_IMPORT_MANIFEST.json').exists()
has_home_audio_r4=(ROOT/'audio/HOME_R4_IMPORT_MANIFEST.json').exists()
if has_home_audio_r4:assert has_playful_r3, 'Home music R4 inherits approved gameplay B and voices'
if has_playful_r3:assert has_batch1c, 'Playful R3 inherits the accepted Batch 1C inputs'
...
if has_result_r13:
    ...
    if has_batch1c:
        ...
        if has_playful_r3:
            playful_tools=runpy.run_path(str(ROOT/'tools/record_playful_build.py'))
            if has_home_audio_r4:
                home_audio_tools=runpy.run_path(str(ROOT/'tools/record_home_build.py'))
                home_audio_before,home_audio_current,_,_=home_audio_tools['verify_scope']()
                playful_before=playful_tools['verify_baseline']()
            else:
                playful_before,playful_current,_,_=playful_tools['verify_scope']()
            batch1c_before=batch1c_tools['verify_historical_chain']()
        else:
...
if audio_import_file.exists():
    ...
    if has_batch1c:
        batch1c_audio_imports=(playful_tools['verify_prior_audio']() if has_playful_r3 else batch1c_tools['verify_audio_imports'](batch1c_before))['entries']
        audio_imports += batch1c_audio_imports
        frames.update(row['uuid'] for row in batch1c_audio_imports)
    if has_playful_r3:
        playful_audio=playful_tools['verify_audio_imports'](playful_before)['entries']
        playful_replacements={r['id']:r for r in playful_audio if r['kind']=='replacement'}
        audio_imports=[playful_replacements.get(r['id'],r) for r in audio_imports]
        audio_imports += [r for r in playful_audio if r['kind']=='new']
        frames.update(row['uuid'] for row in playful_audio)
    if has_home_audio_r4:
        home_audio=home_audio_tools['verify_audio_import']()['entries']
        audio_imports += home_audio
        frames.update(row['uuid'] for row in home_audio)
    assert {str(p.relative_to(project)) for p in (project/'assets').rglob('*.mp3')}=={r['import_path'] for r in audio_imports}
    assert len(audio_imports)==(22 if a3_audio_imports else 18)+(12 if has_batch1c else 0)+(3 if has_playful_r3 else 0)+(1 if has_home_audio_r4 else 0)
...
if has_playful_r3:build_report='review/evidence/playful-r3/BUILD_REPORT.json'
if has_home_audio_r4:build_report='review/evidence/home-r4/BUILD_REPORT.json'
build=json.loads((ROOT/build_report).read_text())
assert build['status']=='success'
if has_home_audio_r4:
    home_audio_tools['verify_recorded_build'](build)
    assert build['runtime_file_sha256']==home_audio_current, 'Home R4 inputs changed after the recorded build'
    assert build['runtime_counts']=={'.ts':12,'.scene':4,'.png':95,'.mp3':38}
    assert build['new_runtime_audio']==['assets/batch1/audio/bgm_home.mp3'] and build['replaced_runtime_audio']==[]
    assert len(expected_scripts)==12 and len(audio_imports)==38
elif has_playful_r3:
    playful_tools['verify_recorded_build'](build)
    assert build['runtime_file_sha256']==playful_current, 'Playful R3 inputs changed after the recorded build'
    assert build['runtime_counts']=={'.ts':12,'.scene':4,'.png':95,'.mp3':37}
    assert set(build['new_runtime_audio'])=={r['import_path'] for r in playful_audio if r['kind']=='new'}
    assert set(build['replaced_runtime_audio'])=={r['import_path'] for r in playful_audio if r['kind']=='replacement'}
    assert len(expected_scripts)==12 and len(audio_imports)==37
elif has_batch1c:
...
report={'status':'passed_integrity_only','source_png_count':len(inventory),
    ...
    'playful_r3_audio_revision':has_playful_r3,
    'home_audio_r4_revision':has_home_audio_r4,

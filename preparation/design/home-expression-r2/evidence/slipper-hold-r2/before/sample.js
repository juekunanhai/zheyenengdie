...
async function start() {
  const [response, skyResponse] = await Promise.all([
    fetch('sequence.json?revision=slipper-grab-r1'), fetch('cloud/manifest.json?revision=livelier-r1')]);
  ...
  document.querySelector('#status').textContent = '先看正常速度：两手接触鸭子后，拖鞋才露出用力表情。可慢放检查抓点和收手。';
  syncPause(); paint();

...
function paintSky(ctx) {
  const bg = images.get(ART + 'home_r10_background.png');
  const scale = Math.max(750 / bg.width, 1334 / bg.height);
  ctx.drawImage(bg, (750 - bg.width * scale) / 2, 1334 - bg.height * scale, bg.width * scale, bg.height * scale);
  const clouds = [[-170, 300, 340, 25, 7, 140, 7, .8], [610, 490, 290, 28, 13, -140, -8, 184 / 255], [-105, 732, 185, 27, 17, 110, 6, .4]];
  for (const [x, y, width, period, offset, travel, lift, alpha] of clouds) {
    const motion = (1 - Math.cos(Math.PI * (state.skyTime + offset) / period)) / 2;
    ctx.save(); ctx.globalAlpha = alpha;
    drawImage(ctx, 'home_r9_cloud', x + travel * motion, y + lift * motion, width);
    ctx.restore();
  }
}
...
async function start() {
  ...
  if (!response.ok) throw new Error('动作素材仍在准备，请稍后刷新。');
  const prepared = await response.json();
  const names = ['home_r10_background', 'home_r9_cloud', 'home_r10_airship', 'home_r10_airplane',
  ...
  document.querySelector('#status').textContent = '请先看正常速度，再用慢放检查眼睑、手腕和抓握。';
  syncPause(); paint();
  // Review-only access: deterministic inspection without editing the sequence or images.
  window.sampleQA = { state, sequence, paint, hero,
    seek: time => { state.time = Math.max(0, Math.min(sequence.duration, time)); state.playing = false; syncPause(); paint(); } };
}

document.querySelector('#replay').addEventListener('click', () => { state.time = 0; state.playing = true; syncPause(); paint(); });
...
document.querySelector('#seek').addEventListener('input', event => { state.time = Number(event.target.value); state.playing = false; syncPause(); paint(); });

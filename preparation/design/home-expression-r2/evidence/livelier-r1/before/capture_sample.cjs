...
const url = 'http://127.0.0.1:8767/preparation/design/home-expression-r2/index.html';
...
async function record(speed) {
  ...
  const snapshots = await page.evaluate(() => new Promise(resolve => {
    const frames = [], start = performance.now(), length = sampleQA.sequence.duration / sampleQA.state.speed;
    const read = now => {
      frames.push({ wall: (now - start) / 1000, scene: sampleQA.state.time });
...
async function inspect() {
  ...
  await page.evaluate(time => sampleQA.seek(time), sequence.reviewTimes.grip);

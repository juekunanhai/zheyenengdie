...
const state = { time: 0, elapsedTime: 0, playing: !matchMedia('(prefers-reduced-motion: reduce)').matches, speed: 1, loop: true };
...
function selectedFrame(frames, localTime, loopFrom) {
  if (localTime < 0) return null;
  const end = frames[frames.length - 1].time;
  if (loopFrom !== undefined && localTime >= end) {
    localTime = loopFrom + (localTime - loopFrom) % (end - loopFrom);
  } else if (localTime > end) return null;
  let frame = frames[0];
...
function paintHero() {
  ...
  paintLayers(ctx, 'hero');
}

function paintLayers(ctx, space) {
  for (const layer of sequence.layers) {
    if ((layer.space ?? 'hero') !== space) continue;
    // Sustained layers play their entrance once, then keep holding across the other actors' loops.
    const clock = layer.loopFrom === undefined ? state.time : state.elapsedTime;
    const frame = selectedFrame(layer.frames, clock - layer.start, layer.loopFrom);
...
function paintSky(ctx) {
  ...
  for (const layer of sky.layers) {
    ...
    const x = (startX + width + 16 + state.elapsedTime * layer.speed) % span - width - 16;
...
function paintDetail(id, rect, source = hero) {
  ...
  ctx.drawImage(source, x, y, width, height, (canvas.width - width * scale) / 2,
...
function paint() {
  ...
  paintDetail('slipper-detail', [250, 370, 290, 325], stage);
...
function animate(now) {
  ...
  if (sequence && state.playing && !document.hidden) {
    ...
    state.elapsedTime += dt * state.speed;
...
async function start() {
  const [response, skyResponse] = await Promise.all([
    fetch('sequence.json?revision=slipper-hold-r2'), fetch('cloud/manifest.json?revision=livelier-r1')]);
  ...
  document.querySelector('#status').textContent = '双手短暂伸出后会一直扒住鸭头，闭眼使劲，嘴持续轻颤；可连续看几轮确认没有松手或恢复原脸。';
  syncPause(); paint();
  // Review-only access: deterministic inspection without editing the sequence or images.
  window.sampleQA = { state, sequence, sky, paint, paintSky, hero,
    seek: time => { state.time = Math.max(0, Math.min(sequence.duration, time)); state.elapsedTime = state.time;
      state.playing = false; syncPause(); paint(); } };
}

document.querySelector('#replay').addEventListener('click', () => { state.time = 0; state.elapsedTime = 0; state.playing = true; syncPause(); paint(); });
...
document.querySelector('#seek').addEventListener('input', event => { state.time = Number(event.target.value); state.elapsedTime = state.time;
  state.playing = false; syncPause(); paint(); });

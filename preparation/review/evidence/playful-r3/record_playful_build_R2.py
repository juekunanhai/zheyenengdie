...
def frozen_path(relative):
    # Text suffix prevents archived TypeScript from joining the project's tsc scan.
    return FROZEN / (relative + '.txt' if relative.endswith('.ts') else relative)
...
def verify_baseline():
    ...
    for name, expected in saved['frozen_files'].items():
        assert expected == saved['files'][name] == sha(frozen_path(name)), 'Frozen baseline input changed: ' + name
...
def verify_scope():
    ...
    old_actions = frozen_path('assets/batch1/scene-actions.ts').read_text()
...
def record(args, prior_input_hashes=None):
    ...
    return {'status': 'success', 'revision': 'playful-r3', 'engine': '3.8.8', 'platform': 'web-desktop',
            ...
            'code_and_settings_sha256': {name: value for name, value in current.items()
                                        if name.endswith('.ts') or name.startswith('settings/') or name in ('package.json', 'tsconfig.json')},
            'scene_source_sha256': {Path(name).name: value for name, value in current.items() if name.endswith('.scene')},

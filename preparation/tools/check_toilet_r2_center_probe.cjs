...
const ROOT=path.resolve(__dirname,'../..'),OUT=path.join(ROOT,'preparation/review/evidence/gameplay-toilet-r2/CENTER_PROXY.json');
...
{id:'W125_F108_H78p5',width:125,foot:108,high:78.5},
{id:'W135_F108_H83p5',width:135,foot:108,high:83.5}];
...
for(const f of ['assets/batch1/object-data.ts','assets/batch1/tower-world.ts','preparation/review/gameplay-lab.js','preparation/tools/check_toilet_r2_center_probe.cjs']) report.sources[f]=crypto.createHash('sha256').update(fs.readFileSync(path.join(ROOT,f))).digest('hex');

...
function draw(name){const data=scenes[name],c=document.getElementById(name),H=+document.querySelector('#ratio').value;c.width=750;c.height=H;const ctx=c.getContext('2d');ctx.fillStyle='#65c4f0';ctx.fillRect(0,0,750,H);const inset=document.querySelector('#safe').checked?{top:48,bottom:32}:{top:0,bottom:0};const safeH=H-inset.top-inset.bottom;
 ...
 if(wi&&(wi._alignFlags&1)&&(wi._alignFlags&4)){h=parent.h-wi._top-wi._bottom;y=parent.y+wi._top}
 if(n._name.startsWith('bg_')&&['bg_ground_city','bg_city_altitude','bg_cloud_altitude','bg_space_altitude'].includes(n._name)){const sp=comps.find(c=>c.__type__==='cc.Sprite'),img=images[frames[sp._spriteFrame.__uuid__].src];h=Math.max(H,750*img.height/img.width);w=h*img.width/img.height;x=(750-w)/2;y=H-h;}
 ctx.save();if(n._name==='World_1x'){const scale=Math.min(1,Math.max(.6,(safeH-240)/1094));const groundY=inset.top+safeH-196;const sourceGroundY=y+1334-196;ctx.translate(375,groundY);ctx.scale(scale,scale);ctx.translate(-375,-sourceGroundY);}
 if(n._name==='Rig'){const scale=Math.min(1,Math.max(.6,(safeH-240)/1094));y-=((safeH-196)/scale-1138);}
 const opacity=comps.find(c=>c.__type__==='cc.UIOpacity');if(opacity)ctx.globalAlpha*=opacity._opacity/255;
 if(n._name==='Content_1230'){const scale=Math.min(1,safeH/1230);ctx.translate(x+w/2,y+h/2);ctx.scale(scale,scale);ctx.translate(-(x+w/2),-(y+h/2));}
 if(n._name==='World_1x'&&document.querySelector('#zoom').checked){ctx.translate(x+w/2,y+h/2);ctx.scale(.75,.75);ctx.translate(-(x+w/2),-(y+h/2))}
 if(n._euler?.z){ctx.translate(x+w/2,y+h/2);ctx.rotate(-n._euler.z*Math.PI/180);ctx.translate(-x-w/2,-y-h/2);}
 ...
 const lb=comps.find(c=>c.__type__==='cc.Label');if(lb){const co=lb._color;ctx.fillStyle=`rgba(${co.r},${co.g},${co.b},${co.a/255})`;ctx.font=`${lb._isBold?'700':'400'} ${lb._fontSize}px sans-serif`;ctx.textAlign='center';ctx.textBaseline='middle';if(lb._enableOutline){ctx.strokeStyle='#134189';ctx.lineWidth=lb._outlineWidth*2;ctx.lineJoin='round';ctx.strokeText(lb._string,x+w/2,y+h/2,w)}ctx.fillText(lb._string,x+w/2,y+h/2,w)}

...
const ROOT=path.resolve(__dirname,'../..'),OUT=path.join(ROOT,'preparation/review/evidence/gameplay-toilet-r2/CENTER_MARGIN_PROXY.json');
...
const variants=[{id:'r1',baseline:true},{id:'W125_F108_H86p5',width:125,foot:108,high:86.5},{id:'W125_F108_H90p5',width:125,foot:108,high:90.5}];
const xs=Array.from({length:7},(_,i)=>-15+i*5);
const holdouts=[
...
const report={status:'running',createdAt:new Date().toISOString(),method:{
 ...
 selection:'Latest user prioritizes forgiving early play. Seek feasible WORLD CENTER x=0 and +/-5,+/-10 for BOTH upright paper and fridge. High cistern platform now crosses world center visibly. Feasible means continuously stable, toilet within 12deg of upright, and payload bottom > toilet bottom+50; falling beside it onto ground is not a success. Holdouts are diagnostic and not used to tune the chosen profile.',
...
for(const f of ['assets/batch1/object-data.ts','assets/batch1/tower-world.ts','preparation/review/gameplay-lab.js','preparation/tools/check_toilet_r2_center_margin_probe.cjs']) report.sources[f]=crypto.createHash('sha256').update(fs.readFileSync(path.join(ROOT,f))).digest('hex');
